...
inputDir_stage1 = "/eos/user/d/dimoulin/analysis_outputs/stage1/"
inputDir_final = "/eos/user/d/dimoulin/plotting/selected_hist/"
jsonDir = "/eos/user/d/dimoulin/nTuples/"

#Get list of signal files names (remove ".root")
root_file_list = os.listdir(inputDir_stage1)
file_name_list = [s[:-5] for s in root_file_list]

#Get cross-sections from dictionnary
with open(jsonDir+"xsec.json", "r") as infile:
    xsec_dict = json.load(infile)
...
lumi_dict = {}
if intLumi == 150e+06:
    key, value = "intLumi", "150 ab-1"
    lumi_dict[key] = value
elif intLumi == 1e+04:
    key, value = "intLumi", "10 fb-1"
    lumi_dict[key] = value
...
def get_max_significance(h_signal, h_list_bg, x1, x2, stage1_path):
    ...
    #x2 : upper bound to compute the significance
    h_significance = h_signal.Clone("h_significance")
    n = get_n_events(stage1_path)
    h_significance.Scale(n/h_significance.Integral())
...
def get_n_events_file(stage1_file):
    file = ROOT.TFile.Open(stage1_file, "READ")
    tree = file.Get("events")
    n_entries = tree.GetEntries()
...
def make_hist_2D(Z_list, selection):
    print("Building 2D histogram..")
    mass_list = []
    coupling_list = []
    significance_list = []

    for Z_info in Z_list:
        x = Z_info[1]
        y = np.log10(Z_info[2]*Z_info[2])
        z = Z_info[0]
        mass_list.append(x)
    ...
    plt.hist2d(mass_list, coupling_list, bins=n_bins, range = [x_range, y_range], weights = significance_list, cmap=cmap, norm=norm)

    hist, x_edges, y_edges = np.histogram2d(mass_list, coupling_list, bins=n_bins, range=[x_range, y_range], weights = significance_list)
    ...
    z_interp = griddata((mass_list, coupling_list), significance_list, (x_grid, y_grid), method='linear')
    plt.contour(x_grid, y_grid, z_interp, levels=[threshold], colors='red', linestyles='dashed')

    plt.xlabel("HNL mass [GeV]")
    plt.ylabel("log(U^2)")
    plt.title("Z-significance (L = " + lumi_dict["intLumi"]+ ")")
    ...
    plt.savefig(output_dir_sel + "Significance_2D"+selection+"_" + "{:.2e}".format(intLumi) +".png")
    plt.close()
...
for selection in selection_list:
    ...
    output_dir_sel =  output_dir + "2Dplots/" + selection +'/'
    ...
    stage1_4body = "/eos/user/d/dimoulin/analysis_outputs/4body_W2023/output_stage1/4body_W2023.root"
    ...
    #Get path for 4body
    file_4body = [input_dir + '4body_W2023_' + selection + '.root', "4-body", 0.0, 0.0102, [0,0], stage1_4body]
    ...
    h_list_bg = []
    body_hist = get_hist(file_4body)
 
    h_list_bg.append(body_hist)
    ...
    #Compute significance for each signal and put it in a list with the mass/coupling information
    Z_list = []
    for file_name in file_name_list:
        mass_legend = file_name[14:24]
        coupling = float(file_name[20:29])
        mass = float(file_name[14:16])
        stage1_file = inputDir_stage1 + file_name + ".root"
        file_path = inputDir_final + file_name + "_" + selection + "_histo.root"
        file_signal = [file_path, mass_legend, coupling, xsec_dict[file_name], [mass-5, mass+5], stage1_file]

        hist = get_hist(file_signal)
        Z = get_max_Z(hist, h_list_bg, file_signal[4][0], file_signal[4][1])
        Z_infos = [Z, mass, coupling, mass_legend]
        Z_list.append(Z_infos)

    #Create 2D plots from Z_list
    make_hist_2D(Z_list, selection)
