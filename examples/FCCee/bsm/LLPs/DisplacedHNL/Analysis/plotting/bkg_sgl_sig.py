...
chosen_variable = variable_list[5]  # Change this to choose a different variable
...
def rebin_histograms(h_list, n_bins):
    for hist in h_list:
        hist.Rebin(n_bins)
    return h_list

def smooth_histogram(hist, sigma=2):
    hist_smoothed = hist.Clone()
    hist_smoothed.SetDirectory(0)
    hist_smoothed.Smooth(sigma)
...
# Rebin the histograms to increase bin count
n_bins_rebin = 2  # Change this value to adjust rebinning
h_list_signal = rebin_histograms(h_list_signal, n_bins_rebin)
h_list_bg = rebin_histograms(h_list_bg, n_bins_rebin)

# Smooth the histograms
h_list_signal = [smooth_histogram(h) for h in h_list_signal]
h_list_bg = [smooth_histogram(h) for h in h_list_bg]

n_bins = h_list_bg[0].GetNbinsX()
x_min = h_list_bg[0].GetXaxis().GetXmin()
x_max = h_list_bg[0].GetXaxis().GetXmax()
h_list_significance = make_significance(h_list_signal, n_bins, x_min, x_max, h_list_bg) 
...
def make_plot(h_list_signal, h_list_bg, legend_list_signal, legend_list_bg, h_list_significance):
    ...
    pad2.SetFillColor(0)
    pad2.SetTopMargin(0.05)  # Adjusted to provide more space for the significance plot
    pad2.SetBottomMargin(0.3)  # Increased to make space for x-axis labels
    pad2.Draw()
    ...
    for i,h in enumerate(h_list_significance):
        ...
        h.GetYaxis().SetTitleSize(0.06)  # Adjusted size
        h.GetYaxis().SetTitleOffset(0.7)  # Adjusted offset
        h.GetYaxis().SetLabelSize(0.1)  # Adjusted label size
        h.GetYaxis().SetLabelOffset(0.01)  # Adjusted label offset
