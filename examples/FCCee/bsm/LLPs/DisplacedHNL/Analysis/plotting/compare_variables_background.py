...
files_list_signal = [
   [file_HNL_20GeV, "RecoElectron_DiJet_delta_R", "20GeV HNL"],
   [file_HNL_50GeV, "RecoElectron_DiJet_delta_R", "50GeV HNL"],
   [file_HNL_70GeV, "RecoElectron_DiJet_delta_R", "70GeV HNL"],


]
files_list_bg = [
   [file_Zbb, "RecoElectron_DiJet_delta_R", "Z -> bb"],
   [file_Zcc, "RecoElectron_DiJet_delta_R", "Z -> cc"],
   [file_4body, "RecoElectron_DiJet_delta_R", "4 body"]
]
...
def make_plot(h_list_signal, h_list_bg, legend_list_signal, legend_list_bg):
   #  print('looking at histogram:', plot_info[0])
    ...
    for ih, h in enumerate(h_list_signal):
        ...
        h_significance_list.append(h_significance.Clone("h_significance_"+str(ih)))       
    
    for ih, h in enumerate(h_significance_list):
        h.SetLineColor(colors_signal[ih])
        h.SetStats(0)
        h.SetMinimum(0)
        h.SetMaximum(h_significance_max*1.25)
