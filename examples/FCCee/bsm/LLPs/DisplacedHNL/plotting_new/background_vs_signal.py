...
def make_plot(h_list_signal, h_list_bg, legend_list_signal, legend_list_bg, h_list_cum_significance):
    ...
    text_lumi.DrawLatexNDC(0.50, 0.72, "#font[52]{#sqrt{s} = 91 GeV , #int L dt = 150 ab^{-1}}")

    pad1.RedrawAxis()

    pad2.cd()
    for i,h in enumerate(h_list_cum_significance):
        
        h.SetLineColor(colors_signal[i])
        h.SetLineWidth(2)
        h.SetStats(0)
        
        h.GetYaxis().SetTitle("Z(#sigma = 10%)")
