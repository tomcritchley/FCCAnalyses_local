...
if __name__ == "__main__":
    ...
    def plot_confusion_matrix(y_true, y_pred, threshold=0.5): #threshold for confusion!
        ...
        plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots13/CM_{file}.pdf")
    ...
    weights_test = np.load(f'/eos/user/t/tcritchl/DNN/testing13/weights_test_{file}.npy', allow_pickle=True)
    ...
    model = tf.keras.models.load_model(f'/eos/user/t/tcritchl/DNN/trained_models13/DNN_HNLs_{file}.keras')
    ...
    plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots13/ROC_{file}.pdf")
    ...
    plt.tight_layout()
    plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots13/Precise_ROC_{file}.pdf")
    plt.close()
    ...
    plt.grid(True)
    plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots13/raw_dnn_classification_{file}.pdf")
    ...
    plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots13/raw_bkg_separated_{file}.pdf")
    ...
    plt.legend(loc='upper center')
    plt.grid(True)
    plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots13/scaled_dnn10fb_{file}.pdf")
    plt.close()
    ...
    plt.legend(loc='upper center')
    plt.grid(True)
    plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots13/scaled_dnn_150ab_{file}.pdf")
    plt.close()
    ...
    plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots13/DNN_output_{file}_10fb.pdf")

    ##################################################################################################################
    ###################################### SAVING MODEL OUTPUTS ######################################################
    ##################################################################################################################

json_file_path = f"/afs/cern.ch/work/t/tcritchl/FCCAnalyses_local/DNN/DNN_condor/DNN_Run13_opt_10fb_{file}.json"
