...
def plot_metrics(history,label):
    ...
    plt.legend()
    plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots12_opt/prec_recall_{label}.pdf")
    """
    def permutation_feature_importance(model, X, y, feature_names,label):
        ...
        plt.tight_layout()
        plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots10_opt/feature_importance_{label}.pdf")
    """
if __name__ == "__main__":
    ...
    tuner.search(X_train, y_train, epochs=60, validation_split=0.2, callbacks=callbacks)

    best_model = tuner.get_best_models(num_models=1)[0]
    best_model.save(f'/eos/user/t/tcritchl/DNN/trained_models12_opt/DNN_HNLs_{args.label}.keras')
    print("Best model saved successfully.")

    history = best_model.fit(X_train, y_train, epochs=60, validation_split=0.2, callbacks=[EarlyStopping(monitor='val_loss', patience=40)]) # sample_weight=weights_train
    label = file
