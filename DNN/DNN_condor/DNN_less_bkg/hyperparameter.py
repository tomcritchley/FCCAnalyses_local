...
def build_model(hp):
    ...
    for i in range(hp.Int('num_layers', 3, 5)):
        model.add(Dense(units=hp.Int(f'units_{i}', min_value=64, max_value=512, step=32),
                        activation='relu'))
        model.add(Dropout(0.5))
...
def plot_metrics(history,label):
    ...
    plt.legend()
    plt.savefig(f"/eos/user/t/tcritchl/DNN/DNN_plots10_opt/prec_recall_{label}.pdf")
...
if __name__ == "__main__":
    ...
    best_model.save(f'/eos/user/t/tcritchl/DNN/trained_models10_opt/DNN_HNLs_{args.label}.keras')
