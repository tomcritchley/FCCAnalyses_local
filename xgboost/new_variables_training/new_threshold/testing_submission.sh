...
base_path="/eos/user/t/tcritchl/xgBOOST/testing11"
# Generate labels for the current signal point
labels=()
for mass in $masses; do
    for coupling in $couplings; do
        base_file="test_signal_${mass}_${coupling//Ve/}.root"
...
for label in "${labels[@]}"; do
    ...
    echo "python3 /afs/cern.ch/work/t/tcritchl/FCCAnalyses_local/xgboost/new_variables_training/new_threshold/testing_macro.py --label \"$label\"" >> "$script_file"
    chmod +x "$script_file"
...
output         = bdt_testing_${label}.\$(ClusterId).\$(ProcId).out
error          = bdt_testing_${label}.\$(ClusterId).\$(ProcId).error
log            = bdt_testing_${label}.\$(ClusterId).\$(ProcId).log
