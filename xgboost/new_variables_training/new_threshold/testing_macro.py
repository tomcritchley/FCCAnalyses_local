...
def plot_confusion_matrix(y_true, y_pred, file, threshold=0.5):
    ...
    plt.savefig(f"/eos/user/t/tcritchl/xgboost_plots{run}_thr/CM_{file}.pdf")
...
if __name__ == "__main__":
    ...
    y_pred_np = np.array(y_pred_list).reshape(-1)
    ...
    plt.savefig(f"/eos/user/t/tcritchl/xgboost_plots{run}_thr/ROC_xgboost_{label}_5.pdf")
    ...
    plt.grid(True)
    plt.savefig(f"/eos/user/t/tcritchl/xgboost_plots{run}_thr/raw_bdt_classification_{label}.pdf")
    plt.close()
    ...
    plt.legend(loc='upper center')
    plt.grid(True)
    plt.savefig(f"/eos/user/t/tcritchl/xgboost_plots{run}_thr/scaled_bdt_classification_{label}.pdf")
    plt.close()
    ...
    def make_cumulative_significance(y_pred, y_true, w, direction="RL"):
        ...
        for thr in thresholds:
            ...
            s_cumulative = np.sum(w[(y_true == 1) & mask])
            b_cumulative = np.sum(w[(y_true == 0) & mask])
            sigma_cumulative = b_cumulative * 0.1  # 10% uncertainty
            ...
            if b_cumulative + sigma_cumulative > 0 and b_cumulative >= 0 and sigma_cumulative != 0:
                ...
                sig_list.append((significance, thr))

        return sig_list

    sig_list = make_cumulative_significance(y_pred_np, y_true, w, significance_direction)
    sig_list.sort(key=lambda x: x[1])
    significance_values, bdt_cuts = zip(*sig_list)
    ...
    ax[1].step(bdt_cuts, significance_values, where='post', color='green', linewidth=1.5)
    ...
    max_significance_cut = bdt_cuts[max_significance_index]
    ...
    plt.savefig(f"/eos/user/t/tcritchl/xgboost_plots{run}_thr/BDT_output_{label}_10fb.pdf")
    ...
    signal = [f"/eos/user/t/tcritchl/xgBOOST/testing{run}/test_{label}.root", "signal"] 
