...
def filter_events(df):
    ...
    df = df.Filter(f"n_RecoElectrons==1 && RecoElectron_lead_e > 15 && RecoDiJet_angle < {np.pi} && RecoElectron_DiJet_delta_R < 5 && RecoDiJet_phi < {np.pi} && RecoDiJet_delta_R < 5 && ROOT::VecOps::All(RecoElectronTrack_absD0 > 5)", "Exactly one electron final state with lead electron energy E > 15 GeV and D0 > 5 (prompt decay)")
    return df
